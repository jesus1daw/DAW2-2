...
function comprobar_usuario($nombre, $clave){
	...
	$ins = "select * from usuarios where userName = '$nombre' 
			and password = '$clave'";
	$resul = $bd->query($ins);	
	if($resul->rowCount() === 1){		
		return $resul->fetch();		
...
function cargar_preguntas(){
	$res = leer_config(dirname(__FILE__)."/configuracion.xml", dirname(__FILE__)."/configuracion.xsd");
	$bd = new PDO($res[0], $res[1], $res[2]);
	$ins = "select idPregunta, enunciado from preguntas";
	$resul = $bd->query($ins);	
	if (!$resul) {
		return FALSE;
	}
...
function cargar_respuestas($id){
	$res = leer_config(dirname(__FILE__)."/configuracion.xml", dirname(__FILE__)."/configuracion.xsd");
	$bd = new PDO($res[0], $res[1], $res[2]);
	$ins = "select idRespuesta, texto from respuestas where idPregunta = $id";
	$resul = $bd->query($ins);	
	if (!$resul) {
		return FALSE;
	}
